...
namespace {
    ...
    public:
        ...
        MOCK_METHOD3(Transaction,
                     std::unique_ptr<TxnResponse>(const std::vector<Compare>& comparisons, const std::vector<RequestOp>& success_ops, const std::vector<RequestOp>& failure_ops));
    };
    ...
    TEST_F(CoordinatorTest, TestHeartbeatExpired_Singleton) {
      ...
      auto raw_client_ptr = (MockClient*) mock_client.get();
      Coordinator c(std::move(mock_client));
      c.HandleHeartbeatExpired(*chain_of_one_, "1");
      EXPECT_TRUE(chain_of_one_->members.empty());
    }
    ...
    TEST_F(CoordinatorTest, TestHeartbeatExpired_ExpiredNodeNeverJoined) {
      ...
      auto raw_client_ptr = (MockClient*) mock_client.get();
      Coordinator c(std::move(mock_client));
      c.HandleHeartbeatExpired(*chain_of_three_, "NONEXISTENT_ID");
    ...
    TEST_F(CoordinatorTest, TestHandleNodeJoin_ChainWasNotSingleton) {
      ...
      c.HandleNodeJoin(*chain_of_three_, "4", new_hb.ToJSON());
      EXPECT_EQ(chain_of_three_->head_id,  "1");
      EXPECT_EQ(chain_of_three_->tail_id, "4");
      EXPECT_EQ(chain_of_three_->members.size(), 4);
      EXPECT_EQ(chain_of_three_->members["1"].config->role, chain::kRoleHead);
      EXPECT_EQ(chain_of_three_->members["1"].config->prev, chain::kNoMember);
      EXPECT_EQ(chain_of_three_->members["1"].config->next, "2");
      EXPECT_EQ(chain_of_three_->members["2"].config->role, chain::kRoleMiddle);
      EXPECT_EQ(chain_of_three_->members["2"].config->prev, "1");
      EXPECT_EQ(chain_of_three_->members["2"].config->next, "3");
      EXPECT_EQ(chain_of_three_->members["3"].config->role, chain::kRoleMiddle);
      EXPECT_EQ(chain_of_three_->members["3"].config->prev, "2");
      EXPECT_EQ(chain_of_three_->members["3"].config->next, "4");
      EXPECT_EQ(chain_of_three_->members["4"].config->role, chain::kRoleTail);
      EXPECT_EQ(chain_of_three_->members["4"].config->prev, "3");
      EXPECT_EQ(chain_of_three_->members["4"].config->next, chain::kNoMember);
    }
    ...
    TEST_F(CoordinatorTest, TestHandleNodeJoin_ChainWasSingleton) {
      ...
      EXPECT_EQ(chain_of_one_->members["1"].config->role, chain::kRoleSingleton);
      EXPECT_EQ(chain_of_one_->members["1"].config->prev, chain::kNoMember);
      EXPECT_EQ(chain_of_one_->members["1"].config->next, chain::kNoMember);

      c.HandleNodeJoin(*chain_of_one_, "2", new_hb.ToJSON());
      EXPECT_EQ(chain_of_one_->head_id, "1");
      EXPECT_EQ(chain_of_one_->tail_id, "2");
      EXPECT_EQ(chain_of_one_->members.size(), 2);
      EXPECT_EQ(chain_of_one_->members["1"].config->role, chain::kRoleHead);
      EXPECT_EQ(chain_of_one_->members["1"].config->prev, chain::kNoMember);
      EXPECT_EQ(chain_of_one_->members["1"].config->next, "2");
      EXPECT_EQ(chain_of_one_->members["2"].config->role, chain::kRoleTail);
      EXPECT_EQ(chain_of_one_->members["2"].config->prev, "1");
      EXPECT_EQ(chain_of_one_->members["2"].config->next, chain::kNoMember);
    }
